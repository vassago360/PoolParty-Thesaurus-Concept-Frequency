...
public class conceptFrequency extends JFrame {
	...
	private final Action action_4 = new SwingAction_4();
 	...
 	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					conceptFrequency frame = new conceptFrequency();
	...
	private int getConceptFreq(String concept) {
	     try {
	         // Create matcher on file
	         Pattern pattern = Pattern.compile("\\b" + concept + "\\b", Pattern.CASE_INSENSITIVE);
 	...
 	private MyTreeNode populateTree(SparqlRetriever s, String concept, MyTreeNode treeNode) {
		if (s.getNarrowerPPXConcepts(concept).isEmpty()) {
			...
			treeNode.frequency = getConceptFreq(concept);
			for (String narrowerConcept : s.getNarrowerPPXConcepts(concept)) {
				if (!narrowerConcept.equals(concept)) {
					...
					subTree.frequency = getConceptFreq(narrowerConcept);
	...
	private class SwingAction_2 extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			int returnVal = fc.showOpenDialog(conceptFrequency.this);
